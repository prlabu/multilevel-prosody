...
%% v1 save files into csv format WITH UPDATED TIME VECTOR for easy reading
files = files(contains(files.name, '.ema'),  :);
out_dir = fullfile(path_data, 'mngu0_s1_ema_csv');
if ~exist(out_dir, 'dir'); mkdir(out_dir); end

utt_dir = fullfile(path_data, 'mngu0_s1_lab_1.1.1');   % where the .utt files live

for ifile = 1:height(files)
    fname = files.name{ifile};
    fpath = fullfile(files.folder{ifile}, fname);

    % base ID (e.g. mngu0_s1_0065)
    base = erase(fname, '.ema');
    utt_path = fullfile(utt_dir, [base '.utt']);

    % ---- find silence end offset ----
    silence_end = 0;
    if exist(utt_path, 'file')
        fid = fopen(utt_path, 'r');
        while true
            tline = fgetl(fid);
            if ~ischar(tline); break; end
            % look for "name #" (the initial silence) and parse its "end" value
            if contains(tline, 'name #')
                tok = regexp(tline, 'end\s+([0-9.]+)', 'tokens', 'once');
                if ~isempty(tok)
                    silence_end = str2double(tok{1});
                end
                break
            end
        end
        ...
        warning('No .utt file for %s', base);
    end

    % ---- load EMA file ----
    ema_current = estload(fpath);
    n_samples = size(ema_current,1);

    % ---- subset channels of interest ----
    ema_sub = ema_current(:, chans_of_interest_idxs);

    % ---- build corrected time vector ----
    time = (0:n_samples-1)'/FS_EMA + silence_end;

    % ---- construct table ----
    tbl = array2table([time ema_sub], ...
        'VariableNames', ['time', convertStringsToChars(chans_of_interest)']);

    % ---- save to CSV ----
    out_csv = fullfile(out_dir, [base '_ema.csv']);
    writetable(tbl, out_csv);

    % % ---- also save .npy for Python ----
    % out_npy = fullfile(out_dir, [base '_ema.npy']);
    % writeNPY(table2array(tbl(:,2:end)), out_npy);          % needs npy-matlab
    % % If you don't have npy-matlab, comment the line above and install from:
    % % https://github.com/kwikteam/npy-matlab

    fprintf('Saved %s (%.3fs offset, %d samples)\n', out_csv, silence_end, n_samples);

    % optional test limit
    % if ifile > 10; break; end
end

disp('✅ Done converting and time-correcting all EMA files.');


%% v0 save files into csv format for easy reading
% files = dir(fullfile(ema_dir, '*.ema'));
files = files(contains(files.name, '.ema'),  :);
...
for ifile = 1:height(files)
    ...
    % optional: also save as MAT
    % out_mat = fullfile(out_dir, [base '_ema.mat']);
    % save(out_mat, 'tbl', 'FS_EMA');
...
disp('✅ Done converting all EMA files to CSV.');
